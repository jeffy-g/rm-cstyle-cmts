...
var log = console.log.bind(console.log, "[:test:]");
var warn = console.log.bind(console.log, "[:warn:]"["yellow"]);
var validate = function (text, expectance, rm_ws, report_regex_evaluate_error) {
...
var caseThrow = function (content, msg, report_regex_evaluate_error) {
    var error = null;
    // check type error.
    try {
        // deceive for tsc.
...
var caseCannotProcessed = function (content, msg) {
    var result = rmc(content);
    assert.strictEqual(content, result, "failed case cannot processed");
    log("\u2714"["green"], "passed: " + msg);
};
var stripFile = function (path) {
